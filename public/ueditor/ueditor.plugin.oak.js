...
OAKEditor.prototype = {
	...
	renderCommand: function (command) {
		...
		for (var i = 0; i < command.length; i++) {
			...
			if (!commondUI) {
				html +=
					"<div class='edui-box edui-separator edui-default'></div>";
			} else {
	...
	me.commands[thePlugins] = {
		execCommand: function () {
			var dialog = new UE.ui.Dialog({
				...
				iframeUrl:
					this.options.UEDITOR_HOME_URL +
					"dialogs/oak-form/checkbox.html",
				name: thePlugins,
				editor: this,
				title: "复选框", //弹出框标题
...
baidu.editor.ui.radio = function (editor, list, title) {
	...
	var kfDialog = new UE.ui.Dialog({
		// 指定弹出层路径
		iframeUrl:
			editor.options.UEDITOR_HOME_URL + "dialogs/oak-form/radio.html",
		// 编辑器实例
		editor: editor,
		// dialog 名称
		name: "radio",
	...
	me.addListener("mouseover", function (t, evt) {
		...
		if (oakplugin == thePlugins) {
			var html = popup.formatHtml(
				"<nobr>" +
					target.getAttribute("title") +
					'   单行文输入: <span onclick=$$._edittext() class="edui-clickable">编辑</span>  <span onclick=$$._delete() class="edui-clickable">删除</span></nobr>'
			);
			if (html) {
				popup.getDom("content").innerHTML = html;
				popup.anchorEl = target;
...
baidu.editor.ui.input = function (editor, list, title) {
	...
	var kfDialog = new UE.ui.Dialog({
		// 指定弹出层路径
		iframeUrl:
			editor.options.UEDITOR_HOME_URL + "dialogs/oak-form/input.html",
		// 编辑器实例
		editor: editor,
		// dialog 名称
		name: "input",
	...
	me.commands[thePlugins] = {
		execCommand: function () {
			var dialog = new UE.ui.Dialog({
				...
				iframeUrl:
					this.options.UEDITOR_HOME_URL +
					"dialogs/oak-form/textarea.html",
				name: thePlugins,
				editor: this,
	...
	me.addListener("mouseover", function (t, evt) {
		...
		if (oakplugin == thePlugins) {
			...
			var html = popup.formatHtml(
				"<nobr>" +
					target.getAttribute("title") +
					'  多行文本入: <span onclick=$$._edittext() class="edui-clickable">编辑</span>  <span onclick=$$._delete() class="edui-clickable">删除</span></nobr>'
			);
			if (html) {
				popup.getDom("content").innerHTML = html;
				popup.anchorEl = target;
...
UE.plugins["select"] = function () {
	...
	me.addListener("click", function (t, evt) {
		...
		if (parentSelect) {
			...
			target.getElementsByClassName("oak-field-value")[0].innerHTML =
				selectValue;
			target.getElementsByClassName("oak-select-root")[0].style.display =
				"none";
		}
...
baidu.editor.ui.select = function (editor, list, title) {
	...
	var kfDialog = new UE.ui.Dialog({
		// 指定弹出层路径
		iframeUrl:
			editor.options.UEDITOR_HOME_URL + "dialogs/oak-form/select.html",
		// 编辑器实例
		editor: editor,
		// dialog 名称
		name: "select",
	...
	me.commands[thePlugins] = {
		execCommand: function () {
			var dialog = new UE.ui.Dialog({
				...
				iframeUrl:
					this.options.UEDITOR_HOME_URL +
					"dialogs/oak-form/timeinput.html",
				name: thePlugins,
				editor: this,
				title: "插入日期输入框", //弹出框标题
